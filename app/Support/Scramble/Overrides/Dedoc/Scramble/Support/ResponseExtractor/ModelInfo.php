...
class ModelInfo
{
    ...
    protected function getAttributes($model)
    {
        try {
            ...
            return $this->getAttributesFromModelMetadata($model);
        }
    }

    /**
     * Fallback when the database is unavailable during docs generation.
     *
     * @param  \Illuminate\Database\Eloquent\Model  $model
     * @return \Illuminate\Support\Collection
     */
    protected function getAttributesFromModelMetadata($model)
    {
        $casts = $model->getCasts();
        $attributeNames = collect($model->getFillable())
            ->merge(['id', 'created_at', 'updated_at'])
            ->merge(array_keys($casts))
            ->unique()
            ->values();

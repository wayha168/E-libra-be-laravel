<aside id="dashboardAside" class="app-aside w-64 h-screen flex flex-col flex-shrink-0 relative">
    <div id="asideTopBar" class="flex items-center justify-between gap-2 px-4 py-4 flex-shrink-0" style="border-bottom: 1px solid var(--border)">
        <div class="flex items-center gap-2.5 min-w-0 aside-brand">
            <div class="aside-brand-mark w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm shrink-0">eL</div>
            <div class="aside-brand-text min-w-0">
                <div class="text-sm font-semibold truncate" style="color: var(--text-primary)">e-Libra</div>
                <div class="text-xs truncate" style="color: var(--text-secondary)" data-i18n="dashboard">Dashboard</div>
            </div>
        </div>
        <button id="asideToggleBtn" type="button" class="glass-btn inline-flex items-center justify-center w-9 h-9 rounded-lg transition shrink-0" aria-expanded="true" aria-label="Close sidebar" title="Close sidebar" data-i18n="close_sidebar" data-i18n-attr="aria-label"></button>
    </div>

    <div class="aside-inner flex flex-col flex-1 min-h-0 p-4 pt-3 overflow-hidden">
    <nav class="aside-nav space-y-1 flex-1 min-h-0 overflow-y-auto">
        ...
        <a href="{{ route('dashboard.authors.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('dashboard.authors.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
            ...
            <span data-i18n="authors">Authors</span>
        </a>
        @endif
        ...
        @if(auth()->user()->isAuthor() && !auth()->user()->isAdmin() && !auth()->user()->isSuperAdmin())
            <span data-i18n="my_books">My Books</span>
        @else
            <span data-i18n="books">Books</span>
        @endif
        </a>
        @if(auth()->user()->isStaff())
        <a href="{{ route('dashboard.promotions.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('dashboard.promotions.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6Z" />
            </svg>
            <span data-i18n="promotions">Promotions</span>
        </a>
        @endif
        ...
        <a href="{{ route('dashboard.purchases.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('dashboard.purchases.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
            ...
            <span data-i18n="book_sales">Book Sales</span>
        </a>
        @endif
        ...
        <a href="{{ route('dashboard.purchases.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('dashboard.purchases.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
            ...
            <span data-i18n="purchases">Purchases</span>
        </a>
        @endif
        @if(auth()->user()->isAdmin() || auth()->user()->isSuperAdmin())
        ...
        <div class="pt-3 mt-3" style="border-top: 1px solid var(--border)">
            <div class="text-[10px] uppercase tracking-widest font-semibold px-3 mb-1" style="color: var(--text-muted)" data-i18n="account_settings">Account Settings</div>
            ...
            <a href="{{ route('dashboard.account.bank.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('dashboard.account.bank.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
                ...
                <span data-i18n="bank_details">Bank Details</span>
            </a>
            @if(auth()->user()->isSuperAdmin())
            <a href="{{ route('dashboard.account.payway.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('dashboard.account.payway.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" /></svg>
                <span data-i18n="aba_payway">ABA PayWay</span>
            </a>
            @endif
            ...
            <a href="{{ route('dashboard.account.activity.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('dashboard.account.activity.*') ? 'bg-black/5 text-black font-medium' : 'text-gray-700 hover:bg-gray-100' }} transition">
                ...
                <span data-i18n="user_activity">User Activity</span>
            </a>
            ...
            <div class="aside-accordion-group" data-accordion="account-settings">
                ...
                <div id="users-submenu" class="aside-accordion-panel overflow-hidden transition-all duration-200 max-h-0" style="max-height:0">
                    <div class="ml-6 mt-1 space-y-0.5 border-l-2 border-gray-200 pl-3">
                        <a href="{{ route('dashboard.users.index') }}" class="block px-2 py-1.5 rounded text-sm {{ request()->routeIs('dashboard.users.index') ? 'text-black font-medium' : 'text-gray-600 hover:text-black hover:bg-gray-50' }} transition" data-i18n="all_users">All Users</a>
                        <a href="{{ route('dashboard.users.create') }}" class="block px-2 py-1.5 rounded text-sm {{ request()->routeIs('dashboard.users.create') ? 'text-black font-medium' : 'text-gray-600 hover:text-black hover:bg-gray-50' }} transition" data-i18n="add_user">Add User</a>
                    </div>
                </div>
            </div>
    ...
    <div class="p-4 pt-0 flex-shrink-0">
        <button id="logoutBtn" class="aside-logout w-full rounded-lg px-4 py-2 font-semibold transition">

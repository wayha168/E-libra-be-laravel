...
<header class="w-full bg-white border-b border-gray-200 sticky top-0 z-10 flex-shrink-0" data-header-hydrated="server">
    <div class="w-full px-6 py-4 flex items-center justify-between gap-4">
        <div class="flex items-center gap-3 min-w-0 pl-0 aside-collapsed:pl-14 transition-[padding] duration-200" id="headerMain">
            <div class="min-w-0">
                <div id="accountName" class="text-sm font-semibold truncate">{{ $headerUser?->name ?? 'Account' }}</div>
            ...
            <div class="relative" id="accountDropdown">
                <button id="accountDropdownBtn" type="button" class="inline-flex items-center gap-2 rounded-xl px-2 py-2 border border-gray-200 bg-white hover:bg-gray-50 transition">
                    <span class="w-8 h-8 rounded-full bg-black/5 flex items-center justify-center">
                        <span id="accountInitial" class="w-7 h-7 rounded-full bg-black text-white flex items-center justify-center text-sm font-semibold">{{ strtoupper(mb_substr($headerUser->name, 0, 1)) }}</span>
                    </span>
                    ...
                    <div class="px-4 py-3 border-b border-gray-100">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-7 h-7 rounded-full bg-black/5 flex items-center justify-center">
                                <span class="text-sm font-semibold text-[#1b1b18]">{{ strtoupper(mb_substr($headerUser->name, 0, 1)) }}</span>
                            </div>
                            <div>
                                <div id="accountEmail" class="text-sm font-semibold text-[#1b1b18]">{{ $headerUser->email }}</div>
                                <div id="accountRole" class="text-xs text-gray-500">{{ $headerUser->display_role }}</div>
                            </div>
                        </div>
                    </div>

...
<body class="min-h-screen bg-white text-[#1b1b18]">
    <div class="p-6 max-w-3xl">
        <h1 class="text-2xl font-semibold mb-4">Edit Image #{{ $image->id }}</h1>
        ...
        <form method="POST" action="{{ route('dashboard.images.update', $image) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm text-gray-600 mb-1">Path</label>
                <input name="path" value="{{ old('path', $image->path) }}" class="w-full border rounded px-3 py-2" />
            </div>

            <div>
                <label class="block text-sm text-gray-600 mb-1">Alt</label>
                <input name="alt" value="{{ old('alt', $image->alt) }}" class="w-full border rounded px-3 py-2" />
            </div>
